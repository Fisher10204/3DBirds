...
struct kuczmamaLab8 : public OpenGLApplicationBase{
	//VisualObject *floor, *pyramid0, *pyramid1, *pyramid2, *pyramid3, *pyramid4;

	kuczmamaLab8() : view(0), rotationX(0.0f), rotationY(0.0f), zTrans(-12.0f)
	{
		floor = new Floor2();
	...
	void setupLighting(GLuint shaderProgram)
	{
		...
		ambOn = true;
		glUniform4fv(colorIntensityLoc, 1, value_ptr(vec4(0.2f, 0.2f, 0.2f, 1.0f)));
		...
		glUniform4fv(directLightColorLoc, 1, value_ptr(vec4(0.75f, 0.75f, 0.75f, 1.0f)));
		glUniform3fv(directLightDirectLoc, 1, value_ptr(normalize(vec3(1.0f, 1.0f, 1.0f))));
		glUniform4fv(directLightSpecLoc, 1, value_ptr(vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		directOn = true;
		...
		GLuint posLightPositionLoc = getUniformLocation( shaderProgram, "posLight.position");
		...
		// Set positional light values
		glUniform4fv(posLightColorLoc, 1, value_ptr(vec4(0.5f, 0.5f, 0.5f, 1.0f)));
		glUniform3fv(posLightPositionLoc, 1, value_ptr(vec3(0.0f, -2.0f, 0.0f)));
		glUniform4fv(posLightSpecLoc, 1, value_ptr(vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		posOn = true;
		...
		GLuint spLightDirectionLoc = getUniformLocation( shaderProgram, 	"spLight.spotDirection");
		GLuint spLightSpecLoc = getUniformLocation( shaderProgram, 	"spLight.specularIntensity");
		GLuint spLightCutoffCosLoc = getUniformLocation( shaderProgram, 	"spLight.spotCutoffCos");
		spOnLoc = getUniformLocation( shaderProgram, "spLight.enabled"	);

		// Set spotlight values
		glUniform4fv(spLightColorLoc, 1, value_ptr(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		glUniform3fv(spLightPositionLoc, 1, glm::value_ptr(glm::vec3(0.0f, 6.0f, 0.0f)));
		glUniform3fv(spLightDirectionLoc, 1, glm::value_ptr(glm::normalize(glm::vec3(0.0f, -1.0f, 0.0f))));
		glUniform4fv(spLightSpecLoc, 1, glm::value_ptr(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		glUniform1f(spLightCutoffCosLoc, std::cos(glm::radians(10.0f)));//10 DEGREES to radians
		spOn = true;
...
void SpecialKeyboardCB(int Key, int x, int y){
	...
	case GLUT_KEY_RIGHT:
		((kuczmamaLab8*)s_pOpenGLAppBase)->rotationY--;
		break;
	case GLUT_KEY_LEFT:
		((kuczmamaLab8*)s_pOpenGLAppBase)->rotationY++;
		break;
	case GLUT_KEY_UP:
		((kuczmamaLab8*)s_pOpenGLAppBase)->rotationX++;
		break;
	case GLUT_KEY_DOWN:
		((kuczmamaLab8*)s_pOpenGLAppBase)->rotationX--;
		break;
...
void kuczmamaLab8::KeyboardCB(unsigned char key, int x, int y){
	switch(key){
	case 'w': case 'W':
		zTrans++;
		break;
...
void viewMenu(int value){
	((kuczmamaLab8*)s_pOpenGLAppBase)->view = value;
	cout << "Changing view " << value << endl;
}//end figureMenu

GLuint kuczmamaLab8::createViewMenu(){
